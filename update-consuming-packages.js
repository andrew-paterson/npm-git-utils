...
module.exports = async function (localConfig) {
  ...
  try {
    ...
    for (const key in displayBranchLockItem) {
      if (!consumingPackagesFiltered.find((item) => item.localRepoPath.endsWith(key)) && !localConfig.dependentPackage.localRepoPath.endsWith(key)) {
    ...
    for (const consumingPackage of consumingPackages) {
      try {
        ...
        if (consumingPackage.commit && consumingPackage.push) {
          ...
          console.log(chalk[consumingPackage.logColour](`[${consumingPackage.name}] code committed but not pushed.`));
        }
